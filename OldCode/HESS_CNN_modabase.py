...
mapped_images,mapped_labels = load_map_data(num_events,location,sum_threshold,cut_nonzero,plot)
single_train_data, single_train_labels, single_test_data, single_test_labels, train_data, train_labels, test_data, test_labels = split_traintest(mapped_images,mapped_labels,plot,normalize)
...
name_str = fnr + "_" + fusiontype + "_" + str_base  + "-base_" + str_num_epochs + "epochs" + str_batch_size + "batchsize" + str_rate + "rate" + str_reg + "reg_" + str_transfer + "transfer_" + str_norm + "_" + formatted_datetime 
name_single_str =  fnr + "_singleviewCNN_" + fusiontype + "_" + str_base  + "-base_" + str_num_epochs + "epochs" + str_batch_size + "batchsize" + str_rate + "rate" + str_reg + "reg_" + str_transfer + "transfer_" + str_norm + "_" + formatted_datetime 
...
if base == 'moda':
    ...
    if transfer == 'yes':
        ...
        input_1 = Input(shape=input_shape)
        cnn_model_1 = create_base_model_customresnet2(input_1, 'Model1', freeze=True)
        cnn_model_1.load_weights('single_cnn_weights_partial.h5', by_name=True)

        input_2 = Input(shape=input_shape)
        cnn_model_2 = create_base_model_customresnet2(input_2, 'Model2', freeze=True)
        cnn_model_2.load_weights('single_cnn_weights_partial.h5', by_name=True)

        input_3 = Input(shape=input_shape)
        cnn_model_3 = create_base_model_customresnet2(input_3, 'Model3',freeze=True)
        cnn_model_3.load_weights('single_cnn_weights_partial.h5', by_name=True)

        input_4 = Input(shape=input_shape)
        cnn_model_4 = create_base_model_customresnet2(input_4,'Model4', freeze=True)
        cnn_model_4.load_weights('single_cnn_weights_partial.h5', by_name=True)
    else: 
        input_1 = Input(shape=input_shape)
