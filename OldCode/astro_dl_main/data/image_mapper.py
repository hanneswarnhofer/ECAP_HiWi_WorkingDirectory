""" image_mapper from dl1_data_handler, see https://github.com/cta-observatory/dl1-data-handler/blob/master/dl1_data_handler/image_mapper.py"""
import numpy as np
import logging
import bisect
...
class ImageMapper:
    ...
    def map_image_back(self, telescope_images, camera_type):
        trans_map_tab = self.mapping_tables[camera_type].transpose()
        vector_size = telescope_images[0].size
        vector = np.array([
            image.T.reshape(vector_size) @ trans_map_tab
            for image in telescope_images
        ])
        return vector
