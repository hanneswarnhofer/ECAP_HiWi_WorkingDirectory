...
fnr = "CustomResNet-MODA-Comparison" 
...
def create_custom_resnet(inputs, model_name,start_layer=-1,freeze=False, dynamic_input_shape=False ):
    ...
    def inner_block(input, filters,outer_nr,inner_nr, strides=(1, 1), firstblock = False,):
        ...
        if firstblock == True: 
            ...
            shortcut = Conv2D(filters, (1, 1), strides=(2,2),kernel_regularizer=l2(reg), padding='same')(shortcut)
        ...
        xd = Conv2D(filters // 4, (3, 3), strides=strides,kernel_regularizer=l2(reg), padding='same',name = xdname)(dropout1)
        ...
        xg = Conv2D(filters, (1, 1), strides=strides,kernel_regularizer=l2(reg), padding='same',name=xgname)(dropout2)
    ...
    #x = outer_block(x,2048,'B3')  CHANGED !!
    #x = Flatten()(x)
    ...
    dense = Dense(units=1024, activation='relu',kernel_regularizer=l2(reg))(dropout)
...
def create_single_model(model):
    ...
    x = Dropout(0.5)(x)
    #x = Flatten(x)
    outputs = Dense(units=1,kernel_regularizer=l2(reg), activation='sigmoid')(x)
